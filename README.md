...
- Importação CSV/Excel para Atendidos, Atividades, Frequência, Atendimentos, Projetos e Grupos possui upload, mapeamento de colunas, pré-visualização, validação de obrigatórios, detecção de duplicados, erros por linha, confirmação antes de salvar e log de importação.
...
O módulo **Importação Bússola** permite alimentar o sistema com planilhas CSV/Excel sem números mockados. Bases suportadas:
...
1. Selecionar a base e fazer upload de CSV/Excel.
2. Revisar o mapeamento entre colunas da planilha e campos Supabase.
3. Conferir a pré-visualização, com dados sensíveis mascarados.
4. Corrigir campos obrigatórios ausentes, UUIDs inválidos, status de frequência inválido e duplicados no arquivo.
5. Confirmar antes de salvar.
6. Registrar log em `logs_importacao`.
7. Após inserção nas tabelas base, Dashboard e Painel Integrado passam a refletir os dados automaticamente porque consomem views agregadas do Supabase.
...
RLS fica ativo nas tabelas base `atendidos`, `atendimentos`, `atividades`, `frequencias`, `projetos`, `grupos` e `relacionamento`. A tela de Importação verifica o perfil autenticado e só libera importação para `administrador` e `coordenacao`; as políticas Supabase continuam protegendo as tabelas base. As views do Painel Integrado são liberadas apenas para `authenticated`; `anon` permanece restrito às views públicas 100% agregadas listadas abaixo.
